...
public class CAlphabetManager extends CNodeManager {
    ...
    public CAlphabetManager( CDasherModel Model, CLanguageModel LanguageModel) {
    	...
    	iChildCount = m_Alphabet.GetNumberSymbols();
    	
    	Log.e("iChildCount ", "iChildCount "+iChildCount);
    	
    //NewChildren	  = new CDasherNode[iChildCount];
    ...
    public CDasherNode GetRoot(CDasherNode Parent, long iLower, long iUpper, int UserData) { // VOID POINTER CHANGED TO INT
    	CDasherNode NewNode = CDasherNode.acquire();
    	 ...
    	 */
    	  ...
    	  if(iSymbol == m_Model.GetSpaceSymbol())
    	    NewNode.set(Parent, iSymbol, 0, EColorSchemes.Special1, iLower, iUpper, m_LanguageModel, iColour);
    	  else
    	    NewNode.set(Parent, iSymbol, 0, EColorSchemes.Nodes1, iLower, iUpper, m_LanguageModel, iColour);
    ...
    public void PopulateChildrenWithSymbol(CDasherNode Node, int iExistingSymbol, CDasherNode ExistingChild) {
    	...
    	/*Log.e("*", "m_Model.GetLongParameter(Elp_parameters.LP_NORMALIZATION) "+
    			m_Model.GetLongParameter(Elp_parameters.LP_NORMALIZATION));*/
    	//(int)m_Model.GetLongParameter(Elp_parameters.LP_NORMALIZATION)) vale 65536
    	LongArrayPool cum = m_Model.GetProbs(Node.Context(), 65536);
     ...
     */    
    CDasherNode[] NewChildren;
    public void PopulateChildrenWithSymbol( CDasherNode Node, int iExistingSymbol, CDasherNode ExistingChild, LongArrayPool cum) {
...
//  	DASHER_TRACEOUTPUT("ChildCount %d\n", iChildCount);
    ...
    // Log.e("iChildCount ","iChildCount "+iChildCount);
    	for(int i = 1; i < iChildCount; i++)
    		cum.array[i] += cum.array[i-1];
    	
    	// create the children
    	EColorSchemes NormalScheme, SpecialScheme;
    	EColorSchemes nodeColorShemes = Node.ColorScheme(); 
    	if(( nodeColorShemes == EColorSchemes.Nodes1) || (nodeColorShemes == EColorSchemes.Special1)) {
    	...
    	// Log.e("CAlphabetManager","iChildCount "+iChildCount);
    	...
    	for(int j = 0; j < iChildCount; j++) {
    		...
    		if(j == ContSymbol)
    			NewNode = m_Model.GetRoot(1, Node, iLbnd, cum.array[j], 0);
    		else if(j == ConvertSymbol) {
    					NewNode = m_Model.GetRoot(0, Node, iLbnd, cum.array[j], 0);
    					NewNode.Seen(false);
    		}
    		else if( j == iExistingSymbol) {
    				NewNode = ExistingChild;
    				NewNode.SetRange(iLbnd, cum.array[j]);
    		}
    		else {
    			...
    			NewNode = CDasherNode.acquire();
    			NewNode.set(Node, j, j, ChildScheme, iLbnd, cum.array[j], m_LanguageModel, iColour);
    			NewNode.m_NodeManager = this;
    			NewNode.m_bShove = true;
    			NewNode.m_BaseGroup = m_Alphabet.m_BaseGroup;
    		}
    ...
    public void Output( CDasherNode Node/*, ArrayList<CSymbolProb> Added, int iNormalization*/) {
    	...
    	if(t != 0) { // Ignore symbol 0 (root node)
    		...
    		PaintActivity2.mActivity.handleEvent(oEvent);
    ...
    public CDasherNode RebuildParent(CDasherNode Node, int iGeneration) {
    	...
    	if(vSymbols.size() <= iGeneration + 1) {
    		...
    		NewNode =CDasherNode.acquire(); 
    		NewNode.set(null, 0, 0,  EColorSchemes.Nodes1, 0, 0, m_LanguageModel, 7);
    		CPPMContext oContext = m_LanguageModel.CreateEmptyContext();
    		...
    		NewNode = CDasherNode.acquire();
    		NewNode.set(null, vSymbols.get(vSymbols.size() - (iGeneration + 2)), 0, ChildScheme, 0, 0, m_LanguageModel, NodeColour);
    	...
    	Log.e("","m_Model.GetBoolParameter(Ebp_parameters.BP_LM_REMOTE) "+m_Model.GetBoolParameter(Ebp_parameters.BP_LM_REMOTE));
    ...
    public void WaitForChildren(CDasherNode node) {
    	while(!(node.GetChildren() != null && node.ChildCount() != 0)) {
    		try {
    			Thread.sleep(50);
    		}
