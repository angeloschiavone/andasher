...
public class CCircleStartHandler extends CStartHandler{
	...
	public boolean DecorateView(CDasherView View) {
		...
		Point C = View.Dasher2Screen(2048, 2048);
		
		Point C2 = View.Dasher2Screen(2048, 2048 + m_iCircleRadius);
	...
	public void Timer(long iTime, CDasherView m_DasherView, CDasherModel m_DasherModel) {
		...
		DPoint dashXY = m_DasherView.getInputDasherCoords();
		
		Point C = m_DasherView.Dasher2Screen(2048, 2048);
		
		Point Cursor = m_DasherView.Dasher2Screen(dashXY.x, dashXY.y);
		dashXY.release();
		double dR;
