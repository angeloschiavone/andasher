...
import com.angelo.androidprova.preferences.MyPreference;
...
/////////////////////////////////////////////////////////////////////////////
     ...
     */
    public CPPMLanguageModel m_LanguageModel;     // pointer to the language model
    /**
...
//	protected CDasherGameMode m_GameMode;
    ...
    protected class CTrainer {
        ...
        public void Train(ArrayList<Integer> vSymbols) {
            ...
            Log.e("m_Context","((CPPMContext)m_Context).head "+((CPPMContext)m_Context).head);
        }
    ...
    public static final int speedIncrease = 60;
    public CDasherModel(CEventHandler EventHandler, CSettingsStore SettingsStore, CDasherInterfaceBase DashIface, CAlphIO AlphIO) {
        ...
        // @todo angelo increase speed +60, why doesn't static param change take effect?
        m_dMaxRate = (GetLongParameter(Elp_parameters.LP_MAX_BITRATE) + speedIncrease ) / 100.0;
        //m_dMaxRate = MyPreference.instance.get
        //m_dMaxRate =  MyPreference.instance.getSpeed();
        
        //Log.e("m_dMaxRate ","m_dMaxRate "+m_dMaxRate);
        ...
        //Log.e("","m_cAlphabet.m_TrainingFile "+m_cAlphabet.m_TrainingFile);
    ...
    public void HandleEvent(CEvent Event) {
    	...
    	//Log.e("CDasherMOdel","HandleEvent "+Event);
    	
        if (Event.m_iEventType == 1) {
            CParameterNotificationEvent Evt = (CParameterNotificationEvent) (Event);

            if (Evt.m_iParameter == Elp_parameters.LP_MAX_BITRATE || Evt.m_iParameter == Elp_parameters.LP_BOOSTFACTOR || Evt.m_iParameter == Elp_parameters.LP_SPEED_DIVISOR) {
     ...
     */
    protected void Make_root(CDasherNode whichchild) //	find a new root node
    {
        ...
        if (t < (m_DasherInterface.GetAlphabet().GetNumberTextSymbols())) {
           ...
           /* @todo angelo uncomment*/ m_LanguageModel.learnSymbol(LearnContext, t);
        }
...
/////////////////////////////////////////////////////////////////////////////
    ...
    public void SetContext(String sNewContext) {
        ...
        CPPMContext therootcontext = m_LanguageModel.CreateEmptyContext();
        ...
        if (sNewContext.length() == 0) {
            ...
            Log.e("** GetLongParameter(Elp_parameters.LP_NORMALIZATION)",""+GetLongParameter(Elp_parameters.LP_NORMALIZATION));
        ...
        ((CPPMContext)LearnContext).release();
    ...
    protected GNRCReturn Get_new_root_coords(long Mousex, long Mousey) {
        ...
        GNRCReturn retval = GNRCReturn.acquire();
...
// works out next viewpoint
            ...
            iNewMin = retval.iNewMin;
            iNewMax = retval.iNewMax;
            retval.release();
...
//		Now actually zoom to the new location
        ...
        *  total_nats += -1.0 * Math.log((iNewMax - iNewMin) / 4096.0);
	
        Log.e("CdasherMOdel","total_nats "+total_nats);
        */
        return true;
    }
...
//				push at a random node on the RHS
    ...
    protected void RecursiveOutput(CDasherNode Node /*, ArrayList<CSymbolProb> pAdded*/) {
        ...
        if (Node.Parent() != null) {
            Node.Parent().m_NodeManager.Leave(Node.Parent());
        }

        Node.m_NodeManager.Enter(Node);

        Node.Seen(true);
        Node.m_NodeManager.Output(Node/*, pAdded, (int) (1 << 16) GetLongParameter(Elp_parameters.LP_NORMALIZATION)*/);
    }
...
//	public PNGCReturn Plan_new_goto_coords(int iRxnew, long mousey)
     ...
     */
    public LongArrayPool GetProbs(CContextBase context, int iNorm) {
    	...
    	LongArrayPool probs  = m_LanguageModel.GetProbs(context, getNonUniformNorm(iNorm));
     ...
     */
    public void adjustProbs(LongArrayPool probs) {
    	
        for (int k = 1; k < probs.array.length; ++k) {
            probs.array[k] += uniformAdd;
        }
        
        probs.array[probs.array.length - 1] = ControlSpace;
...
//		 Total number of symbols
    ...
    protected void Push_Node(CDasherNode Node) {
        ...
        if (Node.HasAllChildren()) {
        	...
        	CDasherNode[] children = Node.GetChildren();
        ...
        if (Node.Context() == null) {
        	CPPMContext cont;
            // sym0
            int nodeSymbol = Node.Symbol();

         //   if(Parent.Context() == null)
         //   Log.e("Parent.Context()",""+(Parent.Context() != null));
            if( (Node.Symbol() < m_cAlphabet.GetNumberTextSymbols() && nodeSymbol > 0) 
            		&(Node.Parent().Context()!=null)) {
            	CDasherNode Parent = Node.Parent();
               // assert (Parent != null);
                // Normal symbol - derive context from parent
                // Log.e("Parent.Context()",""+(Parent.Context() != null));
                cont = m_LanguageModel.CloneContext(Parent.Context());
                m_LanguageModel.EnterSymbol(cont, nodeSymbol);
            } else {
    ...
    protected void Recursive_Push_Node(CDasherNode Node, int iDepth) {
        ...
        for (int i = 0; i < Node.ChildCount(); i++) {
            Recursive_Push_Node(Node.GetChildren()[i], iDepth - 1);
        }
    }
    ...
    public boolean RenderToView(CDasherView View, boolean bRedrawDisplay) {
        ...
        if (!GetBoolParameter(Ebp_parameters.BP_OLD_STYLE_PUSH)) {
        	int vNodeListSize = vNodeList.size();
        	//Log.e("","(m_Root.Context()!=null) "+(m_Root.Context()!=null));
        	for (int i=0;i<vNodeListSize;i++) {
        		//Log.e("","(vNodeList.get(i).Parent().Context()!=null) "+(vNodeList.get(i).Parent().Context()!=null));
        		
                Push_Node(vNodeList.get(i));
            }
        }
        
        int vDeleteListSize = vDeleteList.size();
    	for (int i=0;i<vDeleteListSize;i++) {
            vDeleteList.get(i).Delete_children();
        }
    ...
    public boolean CheckForNewRoot(CDasherView View) {
        ...
        CDasherNode[] children = m_Root.GetChildren();
    ...
    public void deleteLM() {
        if (m_LanguageModel != null) {
            m_LanguageModel.UnregisterComponent();
        }
    }
     ...
     */
    protected CPPMContext CreateEmptyContext() {
